...
int main()
{
    ...
    switch(choose)
    {
        ...
        while (cycele <= chance && !found)
        {
            ...
            if (found)  cout << "居然让你猜对了  QWQ";
            else    cout << "哎呀结束了" << endl << "没事的下次一定可以的  QWQ";
            break;
        }
        break;
        ...
        while (cycele <= chance && !found)
        {
            ...
            if (found)  cout << "你居然猜对了" << endl << "你的实力一定很厉害吧  QWQ";
            else    cout << "哎呀呀   机会用完了哎" << endl << "没事再来一次一定可以的  QWQ";
            break;
        }
        break;

        case 3:
        least = 10,  most = 30, cycele = 1, chance = frequency = 3;
        while (cycele <= chance && !found)
        {
            do
            {
            ...
            if (found) cout << "哦吼!!!" << endl << "居然猜对了" << endl << "你一定玩的是简单模式  哼  QWQ";
            else    cout << "好可惜差一点就猜对了" << endl << "没事好多事情都不是一帆风顺的" << endl << "相信自己下次一定可以的  QWQ" << endl << "我来告诉你正确答案吧" << endl << "是" << num << "没想到吧  嘿嘿" << endl;
            break;
        }
        break;
        ...
        cout << "请输入一个10-25的随机数:";
        cin >> guess;
        if (guess == num)   cout << "快说是不是开了" << endl << "难道这就是传说中的赌神  QWQ" << endl << "但是我还是感觉你开了   嘿嘿QWQ";
        else    cout << "实力不行呀" << endl << "就是逊啊" << endl << "想知道答案吗  QWQ" << endl << "就不告诉你  QWQ";
        break;
        ...
        if (wow == 'Y' || wow == 'y')
        {
            ...
            while (cycele <= chance && !found)
            {
                ...
                if (found)  cout << "恭喜你猜对啦" << endl << "还是自定义符合你的心意呀";
                else    cout << "没关系的设置的太难啦" << endl << "可以在来一遍呀";
                break;
            }
            break;
        }
        else    cout << "祝你游玩开心" << endl << "谢谢";
        break;
    }
}